...
function VRow(props: any): JSX.Element {
  ...
  useEffect(() => {
    const initHeight = (trRef: HTMLTableRowElement | null) => {
      if (trRef?.offsetHeight && !rowHeight && totalLen) {
        let tempRowHeight = trRef?.offsetHeight ?? 0;
        dispatch &&
          dispatch({
            type: 'initHeight',
...
function VTable(props: any): JSX.Element {
  ...
  if (typeof scrollY === 'string') {
    tableScrollY = (wrapTableRef.current?.parentNode as HTMLElement)
      ?.offsetHeight;
  } else {
...
export default function useVirtualTable(
